...
agent_communication:
    - agent: "main"
      message: "PHASE 5 (Opportunity Scoring) backend ready. Login qa.tradescout@example.com / TradeScout!2025. Deterministic 0-100 score, NOT AI. Please test: (1) GET /api/leads -> every lead has lead_score (int 0-100), score_category in [high,good,moderate,low], score_factors object with factors[] of 7 entries. (2) GET /api/leads/:id for a CanadaBuys open tender ('Cell Window Glazing' via ?q=Glazing) -> lead_score should be 90, score_category 'high'. score_factors.factors: active_tender awarded +25 (tender_status 'open'), trade_match +20, contact_info +15, recently_published +15, project_size earned 0 (no source_stated_value), timeline +10, reliable_source +5 (trust 90). Sum of earned (raw) == 90, normalized score == raw since max==100. Each factor has {key,label,points,awarded,earned,reason}. CRITICAL: all factual fields (project_name, tender_status, trade_category, contact_email, timeline_text, etc.) unchanged (scoring only writes lead_score/score_category/score_factors/scored_at/published_at). (3) Find a lead whose tender_status is NOT open/closing_soon (many DEMO or news leads) -> its active_tender factor must be awarded:false, earned:0. (4) Determinism: POST /api/admin/rescore {} twice -> {scored:27,total:27,distribution:{high,good,moderate,low}} and lead_score/score_factors byte-identical between the two runs for the same lead. (5) POST /api/admin/rescore {lead_id:<id>} -> re-scores just that lead. (6) Auth: unauthenticated POST /api/admin/rescore -> 401. (7) No factor awarded without verifiable data: leads with published_at null OR published_at in the future must have recently_published earned:0; leads with source_stated_value null must have project_size earned:0. Report the score + full factor breakdown for 'Cell Window Glazing' and one non-open lead."
    - agent: "testing"
